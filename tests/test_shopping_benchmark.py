...
def test_evaluation_report_aggregates_retrieval_memory_and_provider_metrics() -> None:
    report = evaluate([{
        "case_id": "one", "duration_ms": 100, "cache_hit": True,
        "expected_item_ids": ["a"], "keyword_top3": ["b"], "hybrid_top3": ["a"],
        "expected_memory_keys": ["budget"], "recalled_memory_keys": ["budget"],
        "tool_calls": [{"status": "ok"}], "provider_attempts": [{"platform": "jingdong", "status": "ok"}],
        "expect_cancelled": True, "status": "cancelled",
    }])
