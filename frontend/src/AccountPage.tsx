...
import { ArrowLeft, SignOut, Sparkle, Trash } from "@phosphor-icons/react";
...
export function AccountPage({ user, onBack, onLogout }: {
  ...
  const reload = async () => {
    try {
      const memory = await memoryApi.list();
            ...
            <div className="memory-list">
              {memories.map((item) => (
                <article key={item.memory_id}>
                  ...
                  <button aria-label={`删除 ${item.key}`} onClick={() => void removeMemory(item)}><Trash /></button>
                </article>
              ))}
            </div>
          )}
