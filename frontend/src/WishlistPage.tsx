...
function WishlistCard({ item, busy, onDelete, onHistory, onStatus }: { item: WishlistItem; busy: boolean; onDelete: () => void; onHistory: () => void; onStatus: (status: "active" | "purchased") => void }) {
  ...
  return <article className={`wishlist-card ${busy ? "is-busy" : ""}`}>
    ...
    <div className="wishlist-actions"><div className="wishlist-current"><small>当前价格</small><strong>{money(item.current_price, item.currency) || "暂不可用"}</strong><PriceChange item={item} /></div><div className="wishlist-button-row">{validUrl ? <a href={item.product_url!} rel="noopener noreferrer" target="_blank">前往商城 <ArrowSquareOut /></a> : <button disabled title="暂未提供商品链接">前往商城</button>}<button onClick={onHistory}><MagnifyingGlass />价格详情</button><div className="more-wrap"><button aria-expanded={menu} aria-label="更多操作" onClick={() => setMenu((value) => !value)}><DotsThree /></button>{menu && <div className="more-menu">{item.status === "purchased" ? <button onClick={() => onStatus("active")}><ClockCounterClockwise />恢复关注</button> : <button onClick={() => onStatus("purchased")}><ShoppingCart />标记已购买</button>}<button className="danger" onClick={onDelete}><Trash />删除</button></div>}</div></div></div>
  </article>;
}
