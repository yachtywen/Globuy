...
def _candidate(row: dict[str, Any], platform: Platform, data_as_of: str) -> Candidate | None:
    if platform == "taobao":
        ...
        price = _number(
            row.get("discntPriceYuan") or row.get("priceZKYuanDouble") or row.get("priceYuanDouble")
        )
        ...
        title, price = (
            row.get("title") or row.get("shortTitle"),
            _number(row.get("price") or row.get("lowestPrice")),
        )
        image = _jd_image_url(row.get("imageUrl") or row.get("longImageUrl"))
        url = _absolute_url(row.get("landUrl")) or (
            f"https://item.jd.com/{source_id}.html" if source_id else None
        )
        attributes = {
            key: value
            for key, value in {
        ...
        cents = _number(
            price_data.get("price", {}).get("origin")
            if isinstance(price_data.get("price"), dict)
            else price_data.get("price")
        )
        price = cents / 100 if cents is not None else None
        image, url = (
            _absolute_url(product.get("main_img") or product.get("white_img")),
            _absolute_url(product.get("detail_url")),
        )
...
class JustOneRealtimeProvider:
    ...
    async def search(self, query: str, platform: Platform, limit: int) -> list[Candidate]:
        ...
        params: dict[str, Any] = {
            ...
            "page": 1,
            "pageSize": limit,
            "token": self.settings.justone_api_token.get_secret_value(),
        }
        ...
        result = [
            candidate
            for row in _items(payload, platform)
            if (candidate := _candidate(row, platform, captured))
        ]
        return [
            candidate.model_copy(update={"retrieval_rank": index})
            for index, candidate in enumerate(result[:limit], 1)
        ]
...
class RealtimeCandidateCache:
    ...
    def put(
        self, query: str, platform: Platform, candidates: list[Candidate], ttl_seconds: int
    ) -> _CacheEntry:
