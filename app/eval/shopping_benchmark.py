"""Aggregate recorded shopping-Agent facts into JSON and Markdown reports."""
...
def evaluate(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    retrieval_platform = defaultdict(
        lambda: Counter(attempted=0, succeeded=0, failed=0)
    )
    for record in records:
        ...
        if isinstance(calls, list):
            ...
            tool_failures += sum(
                1
                for call in calls
                if isinstance(call, dict) and call.get("status") in {"error", "failed"}
            )
        if record.get("expect_cancelled") is True:
            cancelled_expected += 1
            cancelled_ok += record.get("status") == "cancelled"
        retrieval_name = record.get("platform")
        if (
            isinstance(retrieval_name, str)
            and "keyword_top3" in record
            and "hybrid_top3" in record
        ):
            bucket = retrieval_platform[retrieval_name]
            bucket["attempted"] += 1
    ...
    return {
        ...
        "cache": {
            "observed_runs": len(cached),
            "hit_rate": _ratio(sum(item["cache_hit"] for item in cached), len(cached)),
        },
        "top3": {
            "keyword_hit_rate": _ratio(sum(keyword_hits), len(keyword_hits)),
            "hybrid_hit_rate": _ratio(sum(hybrid_hits), len(hybrid_hits)),
        },
        "memory": {
            "recall": _ratio(memory_recalled, memory_expected),
            "false_positive_count": memory_false_positive,
        },
        "agent": {
            "tool_failure_rate": _ratio(tool_failures, tool_calls),
            "cancel_success_rate": _ratio(cancelled_ok, cancelled_expected),
        },
        "retrieval_platforms": {
            name: {
                **counts,
                "success_rate": _ratio(counts["succeeded"], counts["attempted"]),
            }
            for name, counts in sorted(retrieval_platform.items())
        },
        "platforms": {
            name: {
                **counts,
            ...
            for name, counts in sorted(platform.items())
        },
    }
...
def markdown(report: dict[str, Any]) -> str:
    ...
    rows = [
        "# Globuy Shopping Evaluation",
        "",
        f"- Cases: {report['case_count']}",
        "",
        "| Metric | Value |",
        "|---|---|",
        f"| Average latency | {value(report['latency_ms']['average'])} ms |",
        f"| P95 latency | {value(report['latency_ms']['p95'])} ms |",
        f"| Cache hit rate | {value(report['cache']['hit_rate'])} |",
        f"| Keyword Top-3 hit rate | {value(report['top3']['keyword_hit_rate'])} |",
        f"| Hybrid Top-3 hit rate | {value(report['top3']['hybrid_hit_rate'])} |",
        f"| Memory recall | {value(report['memory']['recall'])} |",
        f"| Memory false positives | {report['memory']['false_positive_count']} |",
        f"| Tool failure rate | {value(report['agent']['tool_failure_rate'])} |",
        f"| Cancellation success rate | {value(report['agent']['cancel_success_rate'])} |",
    ]
    if report.get("retrieval_platforms"):
        rows.extend(
            [
                "",
                "## Platform retrieval status",
                "",
                "| Platform | Attempts | Success rate |",
                "|---|---:|---:|",
            ]
        )
        rows.extend(
            f"| {name} | {item['attempted']} | {value(item['success_rate'])} |"
            for name, item in report["retrieval_platforms"].items()
        )
    if report["platforms"]:
        rows.extend(
            [
        ...
        rows.extend(
            f"| {name} | {item['attempted']} | {value(item['success_rate'])} |"
            for name, item in report["platforms"].items()
        )
    return "\n".join(rows) + "\n"
...
def main() -> None:
    ...
    parser.add_argument(
        "--json-output",
        type=Path,
        default=Path("output/eval/shopping-report.json"),
    )
    parser.add_argument(
        "--markdown-output",
        type=Path,
        default=Path("output/eval/shopping-report.md"),
    )
